...
class TestSendingDocs:
    ...
    def test_sending_valid_data_cheque(self, setup):
        ...
        for i in range(6):
            ...
            random_bank_index = random.randint(1, len(bank_account.options) - 1)  # Skip index 0 if it's a placeholder
            bank_account.select_by_index(random_bank_index)

            transit_agent = Select(self.wd.drp_transitagent())
            random_transit_index = random.randint(1, len(transit_agent.options) - 1)
            transit_agent.select_by_index(random_transit_index)
