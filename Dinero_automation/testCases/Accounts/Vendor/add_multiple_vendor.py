...
class TestSendingDocs:
    ...
    def test_sending_valid_data(self, setup):
        ...
        VEND_element = self.driver.find_element(By.XPATH,
                                                 "//div[6]//div[2]//div[1]//div[1]//div[3]")
        VEND_element.click()
        time.sleep(3)
        for i in range(3):
            comapny_names = ["Kinder", "Oreo" , "MRF TYRES"
                             ]
            build_name = ["Kinder fantasy", "Oreo businses", "MRF TYRES"]

            auth_name = ["shantan", "Johnichan", "shibu"]
            ...
            email.send_keys(comapny_names[i] + "@gmail.com")
            ...
            auth_pers.send_keys(auth_name[i])
            ...
            id_num.send_keys(random_string_generator_numbers(5))
            id_expiry = self.ven.id_expiry()
            id_expiry.send_keys("30012029")

            cr_num = self.ven.cr_number()
            cr_num.send_keys(random_string_generator_numbers(5))
            cc_num = self.ven.cc_number()
            cc_num.send_keys(random_string_generator_numbers(6))
