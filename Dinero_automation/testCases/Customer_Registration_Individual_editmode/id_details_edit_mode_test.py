...
class Test_ID_Details_Editmode:
    ...
    #             screenShort.screen_short() + "Edit_ID_test_edit_turn_off_dual_nation_save_double_time.png")
    #         assert False

    def test_ID_edit_valiadting_preview(self,setup):
        responce = []
        # Setup the driver for each iteration
        self.driver = setup
        self.driver.get(self.url)
        ...
        place_of_id_val = self.id.place_of_id_issue_field_req().get_attribute('value')
        id_num_val = self.id.id_number_field_req().get_attribute('value')
        place_of_passport = Select(self.id.place_of_passport_isse_drp_req())
        place_of_passport_val = place_of_passport.first_selected_option.text
        passport_num_val = self.id.passport_numb_field_req().get_attribute('value')

        # Second nation
        nationality_drp = Select(self.id.nationality_drp_req_dual())
        nationality_val = nationality_drp.first_selected_option.text
        place_of_pass_drp = Select(self.id.place_of_pass_issue_drp_req_dual())
        place_of_pass_val = place_of_pass_drp.first_selected_option.text
        passport_num_val_seco = self.id.passport_num_req_dual().get_attribute('value')

        # print("Data in the fields:",id_type_val,place_of_id_val,id_num_val,place_of_passport_val,passport_num_val,nationality_val,place_of_pass_val,passport_num_val_seco)

        self.id.btn_next()
        time.sleep(2)
        self.be.click_id_details_drp()
        time.sleep(2)

        # print("data from the preview:",self.be.id_type_pre(),self.be.place_of_id_issue_pre(),self.be.id_num_pre(),self.be.place_of_passport_issue_pre(),self.be.passport_num_pre(),self.be.nationality_pre(),self.be.place_of_passport_issue_dual_pre(),self.be.passport_num_dual_pre())

        if id_type_val == self.be.id_type_pre():
            print("1:",id_type_val,self.be.id_type_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_id_type.png")
            print("1:", id_type_val, self.be.id_type_pre())
            assert False

        if place_of_id_val == self.be.place_of_id_issue_pre():
            print("2:", place_of_id_val, self.be.place_of_id_issue_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_place_of_id.png")
            print("2:", place_of_id_val, self.be.place_of_id_issue_pre())
            assert False

        if id_num_val == self.be.id_num_pre():
            print("3:", id_num_val, self.be.id_num_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_id_num.png")
            print("3:", id_num_val, self.be.id_num_pre())
            assert False

        if place_of_passport_val == self.be.place_of_passport_issue_pre():
            print("4:", place_of_passport_val, self.be.place_of_passport_issue_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_place_of_passport.png")
            print("4:", place_of_passport_val, self.be.place_of_passport_issue_pre())
            assert False

        if passport_num_val == self.be.passport_num_pre():
            print("5:", passport_num_val, self.be.passport_num_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_passport_num.png")
            print("5:", passport_num_val, self.be.passport_num_pre())
            assert False

#       Second

        if nationality_val == self.be.nationality_pre():
            print("6:", nationality_val, self.be.nationality_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_nationality_val.png")
            print("6:", nationality_val, self.be.nationality_pre())
            assert False

        if place_of_pass_val == self.be.place_of_passport_issue_dual_pre():
            print("7:", place_of_pass_val, self.be.place_of_passport_issue_dual_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_place_of_passport.png")
        ...
        if passport_num_val_seco == self.be.passport_num_dual_pre():
            print("8:", passport_num_val_seco, self.be.passport_num_dual_pre())
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_CI_test_ID_edit_valiadting_preview_passport_num.png")
