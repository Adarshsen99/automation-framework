...
class Test_Beneficiary_Editmode:
    ...
    def test_beneficiary_edit_valiadting_searchbar(self,setup):
        responce = []
        ...
        send_bene.send_keys("stiya")
        time.sleep(2)
        click_bene = self.be.click_beneficicaries_1()
        click_bene.click()

        send_bene_val = send_bene.get_attribute('value')
        print(send_bene_val)

        if send_bene_val != send_bene_val:
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_BENEFICIARY_test_beneficiary_edit_valiadting_searchbar.png")
            assert False

    def test_beneficiary_edit_valiadting_multiplebanks(self,setup):
        responce = []
        # Setup the driver for each iteration
        self.driver = setup
        self.driver.get(self.url)
        ...
        send_bene.send_keys("jinga")
        time.sleep(2)
        click_bene = self.be.click_beneficicaries_1()
        click_bene.click()

        get_beneficiary = self.be.get_beneficiary_details()
        print(get_beneficiary.is_displayed())
        get_beneficiary.click()
        time.sleep(4)

        if get_beneficiary.is_displayed() == True:
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_BENEFICIARY_test_beneficiary_edit_valiadting_multiplebanks.png")
            assert False

    def test_beneficiary_edit_validation_the_fastcash_after_removing_the_benefeciary(self,setup):
        responce = []
        # Setup the driver for each iteration
        self.driver = setup
        self.driver.get(self.url)
        ...
        fast_cash_text_bf = self.be.get_fastcash_location().text
        print("fast_cash_text_bf:",fast_cash_text_bf)

        remove_bene = self.be.remove_beneficiary()
        remove_bene.click()

        fast_cash_text_af = self.be.get_fastcash_location().text
        print("fast_cash_text_af:",fast_cash_text_af)

        if fast_cash_text_bf != fast_cash_text_af:
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_BENEFICIARY_test_beneficiary_edit_validation_the_fastcash_after_removing_the_benefeciary.png")
            assert False
