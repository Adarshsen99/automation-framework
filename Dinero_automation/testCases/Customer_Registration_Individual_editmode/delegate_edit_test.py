...
class Test_Delegate_Editmode:
    ...
    def test_delegate_edit_valiadting_by_same_name(self,setup):
        responce = []
        ...
        delgate_name = f_name_val+" "+m_name_val+" "+l_name_val
        print(delgate_name)
        ...
        search_dele.send_keys(delgate_name)

        self.de.click_delegate().click()
        time.sleep(4)

        print("data:",self.de.getting_delegate_name())
        time.sleep(2)

        if delgate_name != self.de.getting_delegate_name():
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_DELEGATE_test_delegate_edit_valiadting_by_same_name.png")
            assert False
    ...
    def test_delegate_edit_valiadting_search_bar(self,setup):
        ...
        if search_dele_val != search_dele_val:
            assert True
        else:
            self.driver.save_screenshot(
                screenShort.screen_short() + "Edit_DELEGATE_test_delegate_edit_valiadting_search_bar.png")
            assert False

        self.driver.quit()

    def test_delegate_sending_docs(self, setup):
        responce = []
        # Setup the driver for each iteration
        self.driver = setup
        self.driver.get(self.url)
        ...
        self.driver.get("http://www.dinero.local/customer/indi/416")
        time.sleep(2)
        self.pi = Personal_Information_Edit(self.driver)
        self.ci = Contact_Information_Edit(self.driver)
        self.id = Id_details_Edit(self.driver)
    ...
    def test_delegate_validating_sptime_invalid(self, setup):
        responce = []
        # Setup the driver for each iteration
        self.driver = setup
        self.driver.get(self.url)
