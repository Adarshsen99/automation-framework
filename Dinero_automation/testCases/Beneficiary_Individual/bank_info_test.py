...
class Test_Bank_Information:
    ...
    def test_with_add_bank(self, setup):
        ...
        # time.sleep(2)
        # self.bi.click_bank_name().click()
        # time.sleep(2)
        #
        # branch_name.send_keys("kochi")
        # time.sleep(2)
        # self.bi.click_branch_name().click()
        ...
        # Extract and print all text values from the currency dropdown LOV
        def get_currency_lov_text_values(currency_dropdown):
            options = currency_dropdown.options  # Get all options from the dropdown
            currency_values = []

            for option in options:
                text_value = option.text.strip()  # Get the text of each option and strip extra spaces
                currency_values.append(text_value)

            print("Currency LOV Text Values:", currency_values)
            return currency_values

        # Call the function to get and print all text values
        currency_lov_text_values = get_currency_lov_text_values(currency)

        # Select a value from currency dropdown
    ...
    def test_adding_multilple_bank_account(self, setup):
        ...
        bank_name.send_keys("canara")
        time.sleep(2)
        self.bi.click_second_bank().click()
        time.sleep(2)

        branch_name.send_keys("kochi")
        time.sleep(2)
        self.bi.click_second_branch().click()
        acc_no = random_string_generator_numbers_new()
        account_num.send_keys(acc_no)
        confirm_account_num.send_keys(acc_no)
        acc_type.select_by_index(2)
