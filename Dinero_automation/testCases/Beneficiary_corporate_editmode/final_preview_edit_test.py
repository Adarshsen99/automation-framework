...
class Test_Final_preview_edit:
    ...
    def test_preview_error_message(self, setup):

        ## test based on manual reports
        ## if we dont add account type and currency in bank and tries to save in final preview, no proper error messege is found
        ...
        if self.fp.error_message == "required fields must be filled":
            assert True
        else:
            self.driver.save_screenshot(screenShort.screen_short() + "fp_error_messsage_not_proper.png")
            assert False

        ## test failed ( no proper error message)
